...
public class Person extends Point {
     ...
     * 增加五个成员变量
     */
    public int top,bottom,left,right;//每个人有自己的活动边界
    public int campus;//每个人都有所属的学校
     ...
     * 修改构造函数
     */
    public Person(City city, int x, int y, int top, int bottom, int left, int right,int campus) {
        super(x, y);
        this.city = city;
        this.top = top;
        this.bottom = bottom;
    ...
    /**
     * 增加判断函数：是否与person为同一个学校
     */
    public boolean ifSameCampus(Person person){
        if(person.campus==this.campus)
            return true;
        else return false;
    }
    /**
    ...
    private void action() {
         ...
         */
        //横向运动边界
        //若CLOSE_DOOR为真（封校），则横向边界为成员变量right和left
        //否则，横向边界为CITY_WIDTH和0
        if (getX() >= (Constants.CLOSE_DOOR?right:Constants.CITY_WIDTH) || getX() <= (Constants.CLOSE_DOOR?left:0)) {
            moveTarget = null;
            udX = -udX;//触及边界，则改变移动方向
        }
        //纵向运动边界
        //若CLOSE_DOOR为真（封校），则纵向边界为成员变量bottom和top
        //否则，横向边界为CITY_HEIGHT和0
        if (getY() >= (Constants.CLOSE_DOOR?bottom:Constants.CITY_HEIGHT) || getY() <= (Constants.CLOSE_DOOR?top:0)) {
            moveTarget = null;
            udY = -udY;
        }
        moveTo(udX, udY);
    ...
    public void update() {
        ...
        for (Person person : people) {
            ...
            //增加判断条件，若封校只能同校感染（使界面内的校墙真正隔绝病毒）
            if (random < Constants.BROAD_RATE && distance(person) < SAFE_DIST
                    && (Constants.CLOSE_DOOR?ifSameCampus(person):true)) {
                this.beInfected();
                break;
            }
        }
